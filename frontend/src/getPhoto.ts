...
async function getPhoto(name: string) {
    const response = await fetch("https://en.wikipedia.org/w/api.php?action=query&prop=pageimages&format=json&piprop=original&origin=*&titles="+name, { method: 'GET', mode: 'cors'});
