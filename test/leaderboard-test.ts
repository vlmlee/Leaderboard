...
describe('Leaderboard', function () {
    ...
    async function deployFixture() {
        ...
        const leaderboard = await Leaderboard.deploy(
            ethers.utils.formatBytes32String('Leaderboard Fixture'),
        ...
        const addRankingTx1 = await leaderboard.addRanking(
            testRankings[0].rank,
            testRankings[0].name,
            testRankings[0].data
        );
        await addRankingTx1.wait();
        const addRankingTx2 = await leaderboard.addRanking(
            testRankings[1].rank,
            testRankings[1].name,
            testRankings[1].data
        );
    ...
    async function deployAllocateRewardFixture() {
        ...
        const leaderboard = await Leaderboard.deploy(
            ethers.utils.formatBytes32String('Allocate Reward Fixture'),
            new Date('12/12/2022').getTime(),
            commissionFee,
            { value: initialFunding }
        );
    ...
    async function deployAllocateInitialRewardFixture() {
        ...
        const leaderboard = await Leaderboard.deploy(
            ethers.utils.formatBytes32String('Allocate Initial Reward Fixture'),
            new Date('12/12/2022').getTime(),
            commissionFee,
            { value: initialFunding }
        );
    ...
    async function deployAllocateAllRewardsFixture() {
        ...
        const leaderboard = await Leaderboard.deploy(
            ethers.utils.formatBytes32String('Allocate All Rewards Fixture'),
            new Date('12/12/2022').getTime(),
            commissionFee,
            { value: initialFunding }
        );
    ...
    describe('Initial balance', async function () {
        it('should be funded with an initial balance for the reward pool', async function () {
            ...
            expect(
                await provider.getBalance(leaderboard.address),
                'Contract balance does not equal the funding amount'
            ).to.equal(initialFunding);
    ...
    describe('Get rankings', async function () {
        it('should get a ranking given a rank', async function () {
            ...
            expect(elonRanking.startingRank, "Elon's starting rank did not match test rank").to.equal(
                elonTestRanking.startingRank
            );
            expect(elonRanking.data, "Elon's data did not match test data").to.equal(
                ethers.utils.hexlify(elonTestRanking.data)
            );
            ...
            expect(jeffRanking.startingRank, "Jeff's starting rank did not match test rank").to.equal(
                jeffTestRanking.startingRank
            );
            expect(jeffRanking.data, "Jeff's data did not match test data").to.equal(
                ethers.utils.hexlify(jeffTestRanking.data)
            );
        });
    ...
    describe('Add rankings', async function () {
        ...
        it('should revert if a name is not provided when adding a new ranking', async function () {
            ...
            await expect(
                leaderboard.addRanking(errorRanking.rank, errorRanking.name, errorRanking.data)
        ...
        it('should not allow any address other than the facilitator to add a new ranking', async function () {
            ...
            await expect(
                leaderboard.connect(addr1).addRanking(errorRanking.rank, errorRanking.name, errorRanking.data)
        ...
        it('should allow arbitrary data in the data field', async function () {
            ...
            await expect(leaderboard.addRanking(testRanking.rank, testRanking.name, testRanking.data))
                ...
                .withArgs([
                    ...
                    testRanking.name,
                    testRanking.rank,
                    testRanking.startingRank,
                    ethers.utils.hexlify(testRanking.data)
                ]); // structs are returned as an array
        });
    });
    ...
    describe('Remove rankings', async function () {
        it('should remove a ranking', async function () {
            ...
            const removeRankingTx = await leaderboard.removeRanking(
                elonTestRanking.id,
                elonTestRanking.rank,
                elonTestRanking.name
            );
        ...
        it('should not be able to remove a ranking if the user is not the facilitator', async function () {
            ...
            await expect(
                leaderboard.connect(addr1).removeRanking(jeffTestRanking.id, jeffTestRanking.rank, jeffTestRanking.name)
            ).to.be.revertedWith('UserIsNotFacilitator');
        });
        ...
        it('should emit RankingRemoved event when a ranking is deleted', async function () {
            ...
            await expect(leaderboard.removeRanking(jeffTestRanking.id, jeffTestRanking.rank, jeffTestRanking.name))
                ...
                .withArgs([
                    jeffTestRanking.id,
                    jeffTestRanking.name,
                    jeffTestRanking.rank,
                    jeffTestRanking.startingRank,
                    ethers.utils.hexlify(jeffTestRanking.data)
                ]);
        });
    ...
    describe('Update rankings', async function () {
        it('should be able update a ranking', async function () {
            ...
            const updateRankingTx = await leaderboard.swapRank(
                fromRanking.id,
                fromRanking.rank,
                toRanking.id,
                toRanking.rank
            ...
            expect(postFromRanking.startingRank, 'From ranking starting rank changed').to.equal(
                fromRanking.startingRank
            );
        ...
        it('should not be able to update a ranking to a zero ranking', async function () {
            ...
            await expect(leaderboard.swapRank(fromRanking.id, fromRanking.rank, toRanking.id, 0)).to.be.revertedWith(
                'RankNeedsToBeGreaterThanOne'
            );
            await expect(leaderboard.swapRank(fromRanking.id, 0, toRanking.id, toRanking.rank)).to.be.revertedWith(
                'RankNeedsToBeGreaterThanOne'
            );
        });
        ...
        it('should not be able to update a ranking to or from a nonexistent ranking', async function () {
            ...
            await expect(leaderboard.swapRank(fromRanking.id, fromRanking.rank, 20, 16)).to.be.revertedWith(
                'RankingDoesNotExist'
            );
            await expect(leaderboard.swapRank(20, 16, fromRanking.id, fromRanking.rank)).to.be.revertedWith(
                'RankingDoesNotExist'
            );
        });
        ...
        it('should emit RankingUpdated event when a ranking is updated', async function () {
            ...
            await expect(leaderboard.swapRank(fromRanking.id, fromRanking.rank, toRanking.id, toRanking.rank))
                ...
                .withArgs([
                    ...
                    toRanking.rank,
                    fromRanking.startingRank,
                    ethers.utils.hexlify(fromRanking.data)
                ])
                ...
                .withArgs([
                    ...
                    toRanking.name,
                    fromRanking.rank,
                    toRanking.startingRank,
                    ethers.utils.hexlify(toRanking.data)
                ]);
        });
        ...
        it('should revert if the id and rankings are invalid', async function () {
            ...
            await expect(
                leaderboard.swapRank(fromRanking.id, fromRanking.rank, toRanking.id, toRanking.rank)
        ...
        it('should revert if updating the name of a nonexistent ranking', async function () {
            ...
            await expect(leaderboard.updateName(testRanking.id, EMPTY_STRING)).to.be.revertedWith(
                'RankingDoesNotExist'
            );
        });
        ...
        it('should revert if updating a ranking name with the same existing name', async function () {
            ...
            await expect(leaderboard.updateName(testRanking.id, testRanking.name)).to.be.revertedWith(
                'RankingNameSuppliedIsTheSame'
            );
        });
        ...
        it('should revert if updating the data of a nonexistent ranking', async function () {
            ...
            await expect(leaderboard.updateData(testRanking.id, testRanking.data)).to.be.revertedWith(
                'RankingDoesNotExist'
            );
        });
    ...
    describe('Add stakes', async function () {
        it('should allow a user to add a stake and increase the reward pool when a new stake is added', async function () {
            ...
            const stakeTx = await leaderboard.connect(addr1).addStake(testRanking.id, testRanking.name, {
                value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
            });
            await stakeTx.wait();
            ...
            expect(
                ...
                'Addr1 balance is not less than before'
            ).to.be.lessThan(+ethers.utils.formatEther(initialAddrBalance));
            expect(
                +ethers.utils.formatEther(await provider.getBalance(leaderboard.address)),
                'Leaderboard contract balance is not greater than before'
            ...
            expect(await leaderboard.rewardPool(), 'Reward pool did not update').to.equal(
                BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(ethers.utils.parseEther(initialFundingAmount))
            ); // reward pool return value in wei
        ...
        it('should revert if a user cannot pay the commission fee', async function () {
            ...
            await expect(
                leaderboard.connect(addr2).addStake(testRanking.id, testRanking.name, {
                    value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
                })
        ...
        it('should revert if a user is trying to stake onto an invalid id', async function () {
            ...
            await expect(
                leaderboard.connect(addr2).addStake(20, testRanking.name, {
                    value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
                })
            ).to.be.revertedWith('RankingDoesNotExist');
            await expect(
                leaderboard.connect(addr2).addStake(3, testRanking.name, {
                    value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
                })
            ).to.be.revertedWith('RankingDoesNotExist');
        });
        ...
        it('should revert if a user is trying to stake onto a ranking passing in the incorrect name', async function () {
            ...
            await expect(
                leaderboard.connect(addr2).addStake(testRanking.id, ethers.utils.formatBytes32String('Someone'), {
                    value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
                })
            ).to.be.revertedWith('RankingDoesNotExist');
        });
        ...
        it('should revert if a user has already staked for this ranking', async function () {
            ...
            await expect(
                leaderboard.connect(addr1).addStake(testRanking.id, testRanking.name, {
                    value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
                })
        ...
        it('should emit a UserStakeAdded event', async function () {
            ...
            expect(await leaderboard.rewardPool(), 'Reward pool did not update').to.equal(
                BigNumber.from(rewardPool).add(ethers.utils.parseEther(stakeAmount))
            ); // reward pool return value in wei
        });
    });
    ...
    describe('Withdraw stakes', async function () {
        it('should withdraw stake for a user if it exists for a ranking', async function () {
            ...
            expect(
                ...
                'Addr1 balance is not greater than before'
            ).to.be.greaterThan(+ethers.utils.formatEther(initialAddrBalance));
            expect(
                +ethers.utils.formatEther(await provider.getBalance(leaderboard.address)),
                'Leaderboard contract balance is not less than before'
            ).to.lessThan(+ethers.utils.formatEther(initialLeaderboardBalance));

            expect(await leaderboard.userStakesSize(), 'User stake size is not 1').to.equal(1);
            expect(await leaderboard.rewardPool(), 'Reward pool did not update').to.equal(
                BigNumber.from(rewardPool).sub(ethers.utils.parseEther(stakeAmount))
            ); // reward pool return values in wei
        ...
        it("should revert if a user tries withdrawing a stake he doesn't have", async function () {
            ...
            await expect(leaderboard.connect(addr2).withdrawStake(addr2.address, testRanking.id)).to.be.revertedWith(
                'UserHasNotStakedYet'
            );
        });
        ...
        it("should revert if a user tries withdrawing another address's stake", async function () {
            ...
            await expect(leaderboard.connect(addr2).withdrawStake(addr1.address, testRanking.id)).to.be.revertedWith(
                'Transaction sender is neither the owner of the stake or the facilitator.'
            );
        });
        ...
        it('should revert entirely if there are no stakes in the contract', async function () {
            ...
            await expect(leaderboard.connect(addr1).withdrawStake(addr1.address, testRanking.id)).to.be.revertedWith(
                'NoStakesAddedForRankingYet'
            );
            await expect(leaderboard.connect(addr2).withdrawStake(addr2.address, testRanking.id)).to.be.revertedWith(
                'NoStakesAddedForRankingYet'
            );
    ...
    describe('Return stakes', async function () {
        it('should return stakes when a ranking is removed if there are any', async function () {
            ...
            expect(
                +ethers.utils.formatEther(await addr2.getBalance()),
                'Addr2 balance is not less than before'
            ...
            expect(await leaderboard.rankingsSize(), 'Ranking size did not decrease by 1').to.equal(
                rankingsSizePrev - 1
            );
            expect(
                await leaderboard.rewardPool(),
                'Reward pool did not decrease by the amount of stakes removed'
            ).to.be.equal(
                BigNumber.from(rewardPool)
                    .sub(ethers.utils.parseEther(stakeAmount))
                    .sub(ethers.utils.parseEther(stakeAmount))
            );

            expect(
                +ethers.utils.formatEther(afterRankingRemovedBalance1),
                'Addr1 balance is not greater than before'
            ).to.be.greaterThan(+ethers.utils.formatEther(beforeRankingRemovedBalance1));
            expect(
                +ethers.utils.formatEther(afterRankingRemovedBalance1),
                'Addr1 balance is greater than initial value'
            ).to.not.be.greaterThan(+ethers.utils.formatEther(initialAddr1Balance));

            expect(
                +ethers.utils.formatEther(afterRankingRemovedBalance2),
                'Addr2 balance is not greater than before'
            ).to.be.greaterThan(+ethers.utils.formatEther(beforeRankingRemovedBalance2));
            expect(
                +ethers.utils.formatEther(afterRankingRemovedBalance2),
                'Addr2 balance is greater than initial value'
        ...
        it('should not return any stakes if none exist for the ranking', async function () {
            ...
            await expect(
                leaderboard.connect(addr1).addStake(testRanking1.id, testRanking1.name, {
                    value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
                })
            )
                .to.emit(leaderboard, 'UserStakeAdded')
            ...
            await expect(leaderboard.removeRanking(testRanking2.id, testRanking2.rank, testRanking2.name))
                ...
                .withArgs([
                    ...
                    testRanking2.name,
                    testRanking2.rank,
                    testRanking2.startingRank,
                    ethers.utils.hexlify(testRanking2.data)
                ])
    ...
    describe('Allocate rewards', async function () {
        ...
        const originalStakeAmounts = ['1.2', '4.0', '2.56', '2.00091', '8.12', '0.9421', '3.16', '1.66', '0.7878'];
        ...
        it('should calculate the right rank changed delta coefficient', async function () {
            ...
            expect(await leaderboard.leaderboardName()).to.equal(
                ethers.utils.formatBytes32String('Allocate Reward Fixture')
            );
            ...
            expect(postFromRanking.rank, 'From ranking did not change').to.equal(toRanking.rank);
            expect(postFromRanking.startingRank, 'From ranking starting rank should not change').to.equal(
                fromRanking.startingRank
            );

            expect(postToRanking.rank, 'To ranking did not change').to.equal(fromRanking.rank);
            expect(postToRanking.startingRank, 'To ranking starting rank should not change').to.equal(
                toRanking.startingRank
            );
        ...
        it('should calculate the right norm for the reward pool', async function () {
            ...
            for (let i = 0; i < testStakes.length; i++) {
                const tx = await leaderboard
                    .connect(accounts[testStakes[i][0]])
                    .addStake(testStakes[i][1], testStakes[i][2], { value: testStakes[i][3] });
                await tx.wait();
            }
        ...
        it('should allocate stake rewards correctly', async function () {
            ...
            const testStakes = [
                [
                    ...
                    fromRanking.id,
                    fromRanking.name,
                    ethers.utils.parseEther(+originalStakeAmounts[0] - commissionFee + '')
                ],
                [
                    addr1.address,
                    toRanking.id,
                    toRanking.name,
                    ethers.utils.parseEther(+originalStakeAmounts[1] - commissionFee + '')
                ],
                [
                    addr1.address,
                    testRanking.id,
                    testRanking.name,
                    ethers.utils.parseEther(+originalStakeAmounts[2] - commissionFee + '')
                ],
                [
                    addr2.address,
                    fromRanking.id,
                    fromRanking.name,
                    ethers.utils.parseEther(+originalStakeAmounts[3] - commissionFee + '')
                ],
                [
                    addr2.address,
                    toRanking.id,
                    toRanking.name,
                    ethers.utils.parseEther(+originalStakeAmounts[4] - commissionFee + '')
                ],
                [
                    addr2.address,
                    testRanking.id,
                    testRanking.name,
                    ethers.utils.parseEther(+originalStakeAmounts[5] - commissionFee + '')
                ],
                [
                    addr3.address,
                    fromRanking.id,
                    fromRanking.name,
                    ethers.utils.parseEther(+originalStakeAmounts[6] - commissionFee + '')
                ],
                [
                    addr3.address,
                    toRanking.id,
                    toRanking.name,
                    ethers.utils.parseEther(+originalStakeAmounts[7] - commissionFee + '')
                ],
                [
                    addr3.address,
                    testRanking.id,
                    testRanking.name,
                    ethers.utils.parseEther(+originalStakeAmounts[8] - commissionFee + '')
                ]
            ];
            // Total: 24.40831
            ...
            console.log(
                'Expected weights: ',
                expectedWeights.map(w => ethers.utils.formatEther(w))
            );
            // [
            //   '0.958',
            //   '4.39725',
            ...
            console.log(
                'Norm calculated from contract (before multiplying by the precision)',
                ethers.utils.formatEther(calculatedNormFromContract)
            );
            console.log('Norm: ', norm / precision); // 0.97855889548
            // Calculated norm from taking reward pool divided by sum of all weights
            console.log(
                'Norm calculated from reward pool / sumOfAllWeights: ',
                total / +ethers.utils.formatEther(sumOfExpectedWeights)
            ); // 0.9785588954799999
            ...
            const testStakesTotalValue = ethers.utils.formatEther(
                testStakes.reduce((acc: any, cur: any) => {
                    acc = acc.add(cur[3]);
                    return acc;
                }, BigNumber.from(0))
            );
            ...
            expect(Math.round(netChangedSum * 1000000) / 1000000).to.be.equal(
                -Math.round(commissionFee * testStakes.length * 1000000) / 1000000
            );
            ...
            expect(await leaderboard.userStakesSize()).to.equal(0);
            const expectedRewardPool = ethers.utils.parseEther(
                '' + (+initialFunding + commissionFee * testStakes.length)
            ); // Initial funding plus commission fees
            const balance = await waffle.provider.getBalance(leaderboard.address);
            expect(balance.div(100000000), 'Contract balance does not equal the expected reward pool.').to.equal(
                expectedRewardPool.div(100000000)
            );
        });
        ...
        it('should allocate initial funding rewards correctly', async function () {
            ...
            await addRankingTx2.wait();
            const addRankingTx3 = await leaderboard.addRanking(
                testRanking.startingRank,
                testRanking.name,
                testRanking.data
            );
            ...
            const updateRankingTx1 = await leaderboard.swapRank(
                testRanking.id,
                testRanking.startingRank,
                toRanking.id,
                toRanking.startingRank
            ...
            const updateRankingTx2 = await leaderboard.swapRank(
                fromRanking.id,
                fromRanking.startingRank,
                toRanking.id,
                testRanking.startingRank
            ...
            for (let i = 0; i < addStakes.length; i++) {
                const tx = await leaderboard
                    .connect(accounts[addStakes[i][0]])
                    .addStake(addStakes[i][1], addStakes[i][2], { value: addStakes[i][3] });
                await tx.wait();
            }
            ...
            console.log(
                'Expected weights: ',
                expectedWeights.map(e => ethers.utils.formatEther(e))
            );
            ...
            console.log(
                'Norm calculated from contract: ',
                ethers.utils.formatEther(
                    await leaderboard.calculateNorm(filteredStakes, ethers.utils.parseEther(initialFunding))
                )
            );
            expect(
                await leaderboard.calculateNorm(filteredStakes, ethers.utils.parseEther(initialFunding)),
            ...
            const filteredStakesTotalValue = ethers.utils.formatEther(
                filteredStakes.reduce((acc: any, cur: any) => {
                    acc = acc.add(cur[3]);
                    return acc;
                }, BigNumber.from(0))
            );
        ...
        it("should withdraw for users stakes where its ranking didn't change", async function () {
            ...
            await expect(leaderboard.returnStakesForUnchangedRankings())
                ...
                .withArgs(addr1.address, [
                    ...
                    fromRanking.name,
                    BigNumber.from(
                        ethers.utils.parseEther((+originalStakeAmounts[0] - commissionFee).toPrecision(8) + '')
                    )
                ])
                .to.emit(leaderboard, 'UserStakeWithdrawn')
                .withArgs(addr2.address, [
                    ...
                    fromRanking.name,
                    BigNumber.from(
                        ethers.utils.parseEther((+originalStakeAmounts[3] - commissionFee).toPrecision(8) + '')
                    )
                ])
                .to.emit(leaderboard, 'UserStakeWithdrawn')
                .withArgs(addr3.address, [
                    ...
                    fromRanking.name,
                    BigNumber.from(
                        ethers.utils.parseEther((+originalStakeAmounts[6] - commissionFee).toPrecision(8) + '')
                    )
                ]);

            expect(
                await leaderboard.userStakesSize(),
                'Stakes for unchanged rankings did not withdraw correctly.'
            ).to.equal(0);
        });

        it('should allocate all rewards correctly', async function () {
            const { leaderboard, addr1, addr2, addr3, addr4, addr5, addr6, addr7 } = await loadFixture(
                deployAllocateAllRewardsFixture
            );
            expect(await leaderboard.leaderboardName()).to.equal(
                ethers.utils.formatBytes32String('Allocate All Rewards Fixture')
            );
            ...
            const testStakes = [
                [
                    ...
                    ranking3.id,
                    ranking3.name,
                    ethers.utils.parseEther(+stakedAmounts[0] - commissionFee + '')
                ],
                [
                    addr1.address,
                    ranking4.id,
                    ranking4.name,
                    ethers.utils.parseEther(+stakedAmounts[1] - commissionFee + '')
                ],
                [
                    addr1.address,
                    ranking5.id,
                    ranking5.name,
                    ethers.utils.parseEther(+stakedAmounts[2] - commissionFee + '')
                ],
                ...
                [
                    ...
                    ranking4.id,
                    ranking4.name,
                    ethers.utils.parseEther(+stakedAmounts[3] - commissionFee + '')
                ],
                [
                    addr2.address,
                    ranking6.id,
                    ranking6.name,
                    ethers.utils.parseEther(+stakedAmounts[4] - commissionFee + '')
                ],
                [
                    addr2.address,
                    ranking7.id,
                    ranking7.name,
                    ethers.utils.parseEther(+stakedAmounts[5] - commissionFee + '')
                ],
                [
                    addr2.address,
                    ranking8.id,
                    ranking8.name,
                    ethers.utils.parseEther(+stakedAmounts[6] - commissionFee + '')
                ],
                [
                    addr2.address,
                    ranking10.id,
                    ranking10.name,
                    ethers.utils.parseEther(+stakedAmounts[7] - commissionFee + '')
                ],
                ...
                [
                    ...
                    ranking1.id,
                    ranking1.name,
                    ethers.utils.parseEther(+stakedAmounts[8] - commissionFee + '')
                ],
                [
                    addr3.address,
                    ranking2.id,
                    ranking2.name,
                    ethers.utils.parseEther(+stakedAmounts[9] - commissionFee + '')
                ],
                [
                    addr3.address,
                    ranking4.id,
                    ranking4.name,
                    ethers.utils.parseEther(+stakedAmounts[10] - commissionFee + '')
                ],
                [
                    addr3.address,
                    ranking5.id,
                    ranking5.name,
                    ethers.utils.parseEther(+stakedAmounts[11] - commissionFee + '')
                ],
                [
                    addr3.address,
                    ranking8.id,
                    ranking8.name,
                    ethers.utils.parseEther(+stakedAmounts[12] - commissionFee + '')
                ],
                [
                    addr3.address,
                    ranking10.id,
                    ranking10.name,
                    ethers.utils.parseEther(+stakedAmounts[13] - commissionFee + '')
                ],
                ...
                [
                    ...
                    ranking1.id,
                    ranking1.name,
                    ethers.utils.parseEther(+stakedAmounts[14] - commissionFee + '')
                ],
                ...
                [
                    ...
                    ranking2.id,
                    ranking2.name,
                    ethers.utils.parseEther(+stakedAmounts[15] - commissionFee + '')
                ],
                [
                    addr5.address,
                    ranking3.id,
                    ranking3.name,
                    ethers.utils.parseEther(+stakedAmounts[16] - commissionFee + '')
                ],
                [
                    addr5.address,
                    ranking5.id,
                    ranking5.name,
                    ethers.utils.parseEther(+stakedAmounts[17] - commissionFee + '')
                ],
                [
                    addr5.address,
                    ranking6.id,
                    ranking6.name,
                    ethers.utils.parseEther(+stakedAmounts[18] - commissionFee + '')
                ],
                [
                    addr5.address,
                    ranking7.id,
                    ranking7.name,
                    ethers.utils.parseEther(+stakedAmounts[19] - commissionFee + '')
                ],
                [
                    addr5.address,
                    ranking8.id,
                    ranking8.name,
                    ethers.utils.parseEther(+stakedAmounts[20] - commissionFee + '')
                ],
                [
                    addr5.address,
                    ranking9.id,
                    ranking9.name,
                    ethers.utils.parseEther(+stakedAmounts[21] - commissionFee + '')
                ],
                [
                    addr5.address,
                    ranking10.id,
                    ranking10.name,
                    ethers.utils.parseEther(+stakedAmounts[22] - commissionFee + '')
                ],
                ...
                [
                    ...
                    ranking1.id,
                    ranking1.name,
                    ethers.utils.parseEther(+stakedAmounts[23] - commissionFee + '')
                ],
                [
                    addr6.address,
                    ranking5.id,
                    ranking5.name,
                    ethers.utils.parseEther(+stakedAmounts[24] - commissionFee + '')
                ],
                [
                    addr6.address,
                    ranking10.id,
                    ranking10.name,
                    ethers.utils.parseEther(+stakedAmounts[25] - commissionFee + '')
                ],
                ...
                [
                    ...
                    ranking1.id,
                    ranking1.name,
                    ethers.utils.parseEther(+stakedAmounts[26] - commissionFee + '')
                ],
                [
                    addr7.address,
                    ranking2.id,
                    ranking2.name,
                    ethers.utils.parseEther(+stakedAmounts[27] - commissionFee + '')
                ],
                [
                    addr7.address,
                    ranking3.id,
                    ranking3.name,
                    ethers.utils.parseEther(+stakedAmounts[28] - commissionFee + '')
                ],
                [
                    addr7.address,
                    ranking4.id,
                    ranking4.name,
                    ethers.utils.parseEther(+stakedAmounts[29] - commissionFee + '')
                ],
                [
                    addr7.address,
                    ranking5.id,
                    ranking5.name,
                    ethers.utils.parseEther(+stakedAmounts[30] - commissionFee + '')
                ],
                [
                    addr7.address,
                    ranking6.id,
                    ranking6.name,
                    ethers.utils.parseEther(+stakedAmounts[31] - commissionFee + '')
                ],
                [
                    addr7.address,
                    ranking7.id,
                    ranking7.name,
                    ethers.utils.parseEther(+stakedAmounts[32] - commissionFee + '')
                ],
                [
                    addr7.address,
                    ranking8.id,
                    ranking8.name,
                    ethers.utils.parseEther(+stakedAmounts[33] - commissionFee + '')
                ],
                [
                    addr7.address,
                    ranking9.id,
                    ranking9.name,
                    ethers.utils.parseEther(+stakedAmounts[34] - commissionFee + '')
                ],
                [
                    addr7.address,
                    ranking10.id,
                    ranking10.name,
                    BigNumber.from(ethers.utils.parseEther(stakedAmounts[35])).sub(
                        ethers.utils.parseEther('' + commissionFee)
                    )
                ]
            ];
            ...
            expect(
                await leaderboard.userStakesSize(),
                'Stakes for unchanged rankings did not withdraw correctly.'
            ).to.equal(testStakes.length);

            const updateRankingTx1 = await leaderboard.swapRank(
                ranking1.id,
                ranking1.startingRank,
                ranking4.id,
                ranking4.startingRank
            ); // 1 -> 4
            await updateRankingTx1.wait();
            const updateRankingTx2 = await leaderboard.swapRank(
                ranking3.id,
                ranking3.startingRank,
                ranking5.id,
                ranking5.startingRank
            ); // 3 -> 5
            await updateRankingTx2.wait();
            const updateRankingTx3 = await leaderboard.swapRank(
                ranking8.id,
                ranking8.startingRank,
                ranking6.id,
                ranking6.startingRank
            ); // 8 -> 6
            await updateRankingTx3.wait();
            const updateRankingTx4 = await leaderboard.swapRank(
                ranking7.id,
                ranking7.startingRank,
                ranking2.id,
                ranking2.startingRank
            ); // 7 -> 2
            ...
            console.log(
                'Expected weights for reward pool stakes: ',
                expectedWeights.map(e => ethers.utils.formatEther(e))
            );
            ...
            const expectedWeightsForInitialFunding = filteredStakes
                .filter(stake => {
                    return rankingsChanged[stake[1]] > 100;
                })
                .map(stake => {
            ...
            console.log(
                'Expected weights for initial funding reward pool',
                expectedWeightsForInitialFunding.map(e => ethers.utils.formatEther(e))
            );
            ...
            console.log(
                'Sum of expected weights for initial funding reward pool: ',
                ethers.utils.formatEther(sumOfExpectedWeightsForInitialFunding)
            ); // 105.90579999999999469

            console.log('Reward pool for initial funding reward: ', 2.0);
            const normForInitialFundingReward = ethers.utils
                .parseEther('2.0')
                .mul(precision)
                .div(sumOfExpectedWeightsForInitialFunding);
            ...
            allocateTx.events
                .filter(
                    event =>
                        event.event === 'SuccessfullyAllocatedStakeRewardTo' ||
                        event.event === 'SuccessfullyAllocatedInitialFundingRewardTo'
                )
                .sort((a, b) => {
                ...
                .forEach(event => {
                    ...
                    if (event.event === 'SuccessfullyAllocatedStakeRewardTo') {
                        console.log(
                            `Stake redistribution reward => User: ${
                                addresses[event.args['_user']]
                            }, Stake reward returned: ${ethers.utils.formatEther('' + event.args['_reward'])}`
                        );
                    }
                    if (event.event === 'SuccessfullyAllocatedInitialFundingRewardTo') {
                        console.log(
                            `Initial funding winner => User: ${
                                addresses[event.args['_user']]
                            }, Stake reward returned: ${ethers.utils.formatEther('' + event.args['_reward'])}`
                        );
                    }
            ...
            expect(
                allocateTx.events.filter(event => event.event === 'UserStakeWithdrawn').length,
                'All withdraw events did not occur'
            ).to.equal(7);
            // Only positive ranking changes
            expect(
                allocateTx.events.filter(event => event.event === 'SuccessfullyAllocatedInitialFundingRewardTo').length,
                'All initial funding events did not occur'
            ).to.equal(16);
            // Both positive and negative ranking changes
            expect(
                allocateTx.events.filter(event => event.event === 'SuccessfullyAllocatedStakeRewardTo').length,
                'All stake reward events did not occur'
            ).to.equal(29);
            ...
            expect(
                allocateTx.events.filter(event => event.event === 'UserStakeFulfilled').length,
                'All user stake fulfilled events did not occur'
            ).to.equal(29);
    ...
    describe('Contract ended', async function () {
        it('should revert adding stakes if the contract has already ended', async function () {
            ...
            await expect(
                leaderboard
                    .connect(addr1)
                    .addStake(testRanking.id, testRanking.name, { value: ethers.utils.parseEther('1') })
...
describe('Destroy contract', async function () {
    it('should return all stakes to their respective addresses and emit a ContractDestroyed event', async function () {
        ...
        await expect(
            leaderboard.connect(addr1).addStake(testRanking2.id, testRanking2.name, {
                value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
            })
        )
            .to.emit(leaderboard, 'UserStakeAdded')
        ...
        await expect(
            leaderboard.connect(addr2).addStake(testRanking1.id, testRanking1.name, {
                value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
            })
        )
            .to.emit(leaderboard, 'UserStakeAdded')
        ...
        await expect(
            leaderboard.connect(addr2).addStake(testRanking2.id, testRanking2.name, {
                value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
            })
        )
            .to.emit(leaderboard, 'UserStakeAdded')
        ...
        await expect(
            leaderboard.connect(addr3).addStake(testRanking1.id, testRanking1.name, {
                value: BigNumber.from(ethers.utils.parseEther(stakeAmount)).add(commissionFee)
            })
        )
            .to.emit(leaderboard, 'UserStakeAdded')
        ...
        expect(
            +ethers.utils.formatEther(beforeDestroyContractAddr1Balance),
            'Addr1 balance is not less than before the contract is destroyed'
        ).to.be.lessThan(+ethers.utils.formatEther(initialAddr1Balance));
        expect(
            +ethers.utils.formatEther(beforeDestroyContractAddr2Balance),
            'Addr2 balance is not less than before the contract is destroyed'
        ).to.be.lessThan(+ethers.utils.formatEther(initialAddr2Balance));
        expect(
            +ethers.utils.formatEther(beforeDestroyContractAddr3Balance),
            'Addr3 balance is not less than before the contract is destroyed'
        ...
        expect(
            +ethers.utils.formatEther(await addr1.getBalance()),
            'Addr1 balance is not greater than after the contract is destroyed'
        ).to.be.greaterThan(+ethers.utils.formatEther(beforeDestroyContractAddr1Balance));
        expect(
            +ethers.utils.formatEther(await addr2.getBalance()),
            'Addr2 balance is not greater than after the contract is destroyed'
        ).to.be.greaterThan(+ethers.utils.formatEther(beforeDestroyContractAddr2Balance));
        expect(
            +ethers.utils.formatEther(await addr3.getBalance()),
            'Addr3 balance is not greater than after the contract is destroyed'
